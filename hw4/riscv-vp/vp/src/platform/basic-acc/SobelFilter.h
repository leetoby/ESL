    ...
    while (true) {
      ...
      for (unsigned int v = 0; v < MASK_Y; ++v) {
        for (unsigned int u = 0; u < MASK_X; ++u) {
          unsigned int i_r_read = i_r.read();
          unsigned int i_g_read = i_g.read();
          unsigned int i_b_read = i_b.read();
          wait(CLOCK_PERIOD, SC_NS);
          
          val_r += i_r_read * mask[u][v];
          val_g += i_g_read * mask[u][v];
          val_b += i_b_read * mask[u][v];
          wait(CLOCK_PERIOD, SC_NS);
        }
      }
      int total = 0;
