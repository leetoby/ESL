...
void Testbench::do_sobel() {
	...
	for (y = -1; y < height+2; ++y) {
		for (x = 0; x < width; ++x) {
			...
			if (y > 1) {
				*(target_bitmap + bytes_per_pixel * (width * (y - 2) + x) + 2) = i_result_r.read();
				*(target_bitmap + bytes_per_pixel * (width * (y - 2) + x) + 1) = i_result_g.read();
				*(target_bitmap + bytes_per_pixel * (width * (y - 2) + x) + 0) = i_result_b.read();
			} else {
				i_result_r.read();
				i_result_g.read();
				i_result_b.read();
